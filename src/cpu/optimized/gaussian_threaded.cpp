...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < RUNS; i++) {
        timer.start();
        result = gaussian_threaded(src, 15, 2.0);
        timer.stop();
        total_ms += timer.elapsed_ms();
    }

    double avg_ms = total_ms / RUNS;
    std::cout << "Threaded Gaussian (avg " << RUNS << " runs): " 
              << avg_ms << " ms" << std::endl;

    save_image("output_threaded.png", result);
    std::cout << "Saved: output_threaded.png" << std::endl;
